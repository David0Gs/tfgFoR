...
import 'package:prueba_tfg/application/bots/local_bots.dart';
...
void main() {
  group('Local bot controller', () {
    ...
    test('falls back to income when a bot action raises a rule error', () async {
      final Juego game = Juego(
        2,
        playerKinds: const <TipoJugador>[TipoJugador.human, TipoJugador.bot],
      );
    ...
    test('chooses a larger replacement build when overwrite is valid', () {
      ...
      expect(
        bot.availableBuildings.any((Edificio building) => building.id == domus.id),
        isTrue,
      );
      expect(game.indiceJugadorActual, 0);
    });
