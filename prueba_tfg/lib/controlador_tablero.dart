...
/// Controlador centralizado del tablero
/// Coordina interacción entre vista 3D y lógica del juego
class TableroController {
  ...
  /// Verifica si el visor 3D está listo
  bool get visorListo => _visorListo;

  /// Registra callback para clicks en objetos del entorno 3D
  ...
  Future<void> ocultarEdificiosEnCoordenadas(Iterable<String> coords) async {
    ...
    await restaurarEdificiosOcultosPorPreview();
    ...
    for (final String buildingId in idsToHide) {
      await _setModelVisibleIfChanged(buildingId, false);
      _hiddenPreviewBuildingIds.add(buildingId);
    }

    for (final String markerId in markerIdsToHide) {
      await _setModelVisibleIfChanged(markerId, false);
      _hiddenPreviewMarkerIds.add(markerId);
    }
  }
  ...
  double _rotationIndexToRadians(int rotationIndex, {required String templateId}) {
