...
// Función auxiliar para obtener la población que aporta un edificio residencial
...
class Juego {
  ...
  final Map<Era, List<CartaEscritura>> mazos = {Era.I: [], Era.II: [], Era.III: []};
  ...
  Juego(this.numeroJugadores, {List<TipoJugador>? playerKinds})
    ...
    _configurarTablero(); // [cite: 56, 60]
    _configurarJugadores(); //
    _configurarMonumentos(); // Seleccionar monumentos aleatorios
    _configurarMazos(); // [cite: 11]
    _refrescarMercado(); // [cite: 12]
  }
  ...
  void _configurarJugadores() {
    for (int i = 0; i < numeroJugadores; i++) {
      ...
      // Inicializar bandeja solo con edificios NO-monumentos
      p.availableBuildings.addAll(
        catalogoEdificios.where((b) => !conjuntoIdsMonumentos.contains(b.id)),
      );
      players.add(p);
    }
    _actualizarMarcadoresPoblacion(); // Inicializar populationTracks
  }
  ...
  // ACCIÓN A: INGRESOS [cite: 18, 19, 83]
  void accionIngresos() {
  ...
  // ACCIÓN B: COMPRAR ESCRITURA [cite: 21, 23, 24]
  bool comprarParcela(int index) {
    ...
    if (p.coins >= cost) {
      ...
      p.availableMarkers--; // Consumimos el marcador físico
  ...
  // ACCIÓN C: CONSTRUIR / SOBREESCRIBIR CON ROTACIONES [cite: 26, 30, 51, 75]
  ...
  bool construir(
    ...
    // Ejecutar la eliminación de edificios antiguos (vuelven a la reserva) [cite: 31, 76]
    for (var oldB in buildingsToRemove) {
      // Limpiamos las coordenadas que ocupaba el edificio antiguo
  ...
  void _endTurn() {
    ...
    if (eraActual == Era.III && mercado.isEmpty && mazos[eraActual]!.isEmpty) {
      if (!rondaFinalEraIIIActiva) {
        ...
        registrarResumenPendiente('Se han agotado las parcelas de la Era III.', [
  ...
  Map<String, dynamic> toJson({bool includePendingSummary = true}) {
    ...
    if (includePendingSummary) {
      json.addAll(<String, dynamic>{
        ...
        'avanzarAEraSiguienteDespuesDeResumen': avanzarAEraSiguienteDespuesDeResumen,
      });
    }
  ...
  factory Juego.fromJson(Map<String, dynamic> json) {
    ...
    game.eraActual = Era.values.firstWhere(
      (e) => e.name == json['eraActual'],
    );
    ...
    game.propietariosLotes.addAll((json['propietariosLotes'] as Map).cast<String, int>());
    ...
    game.edificios.addAll(
      (json['edificios'] as Map).cast<String, dynamic>().map((k, v) {
        ...
        final List<String> coords =
            (buildingJson['coordenadasOcupadas'] as List).cast<String>().toList()
              ..sort();
  ...
  /// Validación previa para catálogo.
  /// Usa exactamente el mismo motor de validación que la colocación real,
  /// pero probando TODAS las rotaciones hasta encontrar una válida.
  String? puedeColocarEdificio(
    ...
    return lastError ?? 'No hay rotación válida para este edificio en esa zona.';
  }
}
