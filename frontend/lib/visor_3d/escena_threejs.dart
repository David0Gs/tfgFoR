// Generador del documento HTML/JavaScript de Three.js. Este archivo concentra
// la escena 3D que despues se incrusta en web, escritorio y movil.
...
String buildThreeJsHtml(
  ...
  final pixelRatioMax = maxPixelRatio.toStringAsFixed(2);
...
const LOW_POWER_MODE = $lowPowerModeJs;
...
function init() {
  ...
  renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1.5, $pixelRatioMax));
  ...
  const scenarioModelUrl = '$resolvedScenarioModelUrl';
  const initialModelUrl = '$resolvedModelUrl';
...
function renderFrame() {
  ...
  reportPerformanceIfNeeded(performance.now() - frameStart);
  if (
    !RENDER_CONTINUOUSLY &&
...
function reportPerformanceIfNeeded(frameMs) {
  if (IS_THUMBNAIL) {
    return;
  }
  ...
  if (now - perfLastReportAt < PERF_REPORT_INTERVAL_MS) {
    return;
  }

  const modelIds = Object.keys(loadedModels);
  const report = {
    avgFrameMs: perfFrameTotalMs / Math.max(1, perfFrameCount),
    models: modelIds.length,
    built: modelIds.filter((id) => id.startsWith('built_')).length,
    markers: modelIds.filter((id) => id.startsWith('lot_marker_')).length,
    animations: Object.keys(positionAnimations).length,
  ...
  sendMessage('perf:3d:' + JSON.stringify(report));
  perfFrameCount = 0;
  perfFrameTotalMs = 0;
...
const CORE_CACHE_IDS = new Set(['main', 'scenario']);
const LOW_POWER_GLTF_CACHE_LIMIT = 14;
const gltfCache = new Map();
const gltfCacheUseOrder = new Map();
const modelUrlById = new Map();
const tilePositionCache = new Map();
const buildingColliderRoots = new Map();
const BUILDING_COLLIDER_SIZE = 0.92;
const BUILDING_COLLIDER_HEIGHT = 0.75;
const BUILDING_COLLIDER_Y_OFFSET = 0.28;
...
function loadGltfCached(url) {
  ...
  gltfCacheUseOrder.set(url, performance.now());
  return entry.promise;
}

function disposeUnusedGltf(url) {
  if (!url || isGltfUrlInUse(url)) {
    return;
  }

  disposeCachedGltf(url);
}

function isGltfUrlInUse(url) {
  for (const activeUrl of modelUrlById.values()) {
    if (activeUrl === url) {
      return true;
    }
  }
...
function disposeCachedGltf(url) {
  ...
  if (entry.gltf) {
    ...
    entry.gltf.scene.traverse((node) => {
      if (!node.isMesh) {
        return;
      }
  ...
  gltfCache.delete(url);
  gltfCacheUseOrder.delete(url);
  console.log('GLB cache liberada:', url);
}
...
function pruneGltfCacheIfNeeded() {
  if (!LOW_POWER_MODE || gltfCache.size <= LOW_POWER_GLTF_CACHE_LIMIT) {
    return;
  }

  const urlsInUse = new Set(modelUrlById.values());
  const removableUrls = Array.from(gltfCache.keys())
    .filter((url) => !urlsInUse.has(url))
    .sort((a, b) => (gltfCacheUseOrder.get(a) || 0) - (gltfCacheUseOrder.get(b) || 0));

  while (
    gltfCache.size > LOW_POWER_GLTF_CACHE_LIMIT &&
    removableUrls.length > 0
  ) {
    const url = removableUrls.shift();
    disposeCachedGltf(url);
  }
}

function cloneMaterialForInstance(material) {
  if (material.name !== 'playerTint') {
    material.userData = material.userData || {};
    material.userData._sharesCachedMaterial = true;
    return material;
  }
...
function disposeObject3D(object) {
  ...
  object.traverse((child) => {
    ...
    disposeMaterialIfOwned(child.material);
  });
}

function disposeMaterialIfOwned(material) {
  if (!material) {
    return;
  }

  if (Array.isArray(material)) {
    material.forEach(disposeMaterialIfOwned);
    return;
  }

  if (material.userData?._sharesCachedMaterial === true) {
    return;
  }
...
function addMobilePanoramaBackground(texture) {
  ...
  sphere.rotation.y = THREE.MathUtils.degToRad(310);
...
function loadEnvironmentMap() {
  ...
  if (IS_THUMBNAIL || LOW_POWER_MODE) {
    ...
    if (LOW_POWER_MODE && !IS_THUMBNAIL) {
      const textureLoader = new THREE.TextureLoader();
      textureLoader.load(
        MOBILE_BACKGROUND_PATH,
        (texture) => {
          texture.colorSpace = THREE.SRGBColorSpace;
          addMobilePanoramaBackground(texture);
          requestRender();
        },
        undefined,
        (error) => {
          console.error('Error cargando fondo movil:', error);
        }
      );
    }
    return;
  ...
  model.traverse((child) => {
    ...
    for (const material of materials) {
      ...
      if (material.envMapIntensity !== undefined) {
        material.envMapIntensity = LOW_POWER_MODE
          ? LOW_POWER_ENV_MAP_INTENSITY
          : ENV_MAP_INTENSITY;
        material.needsUpdate = true;
      }
    }
...
function loadModel(id, url, x, y, z, onLoaded, options = {}) {
...
function setVisualModelClickable(model, id, clickable) {
  if (!model) {
    return;
  }
...
function disposeColliderGroup(group) {
  const geometries = new Set();
  const materials = new Set();
...
function removeBuildingClickColliders(buildingId) {
  const group = buildingColliderRoots.get(buildingId);
  if (!group) {
    return;
  }

  raycastRoots.delete(group);
  scene.remove(group);
  disposeColliderGroup(group);
  buildingColliderRoots.delete(buildingId);
  requestRender();
}

function syncBuildingClickColliders(buildingId, coordsCsv) {
  removeBuildingClickColliders(buildingId);

  const coords = String(coordsCsv || '')
    .split(',')
  ...
  group.name = buildingId + '_click_colliders';
  group.userData = { clickId: buildingId, clickable: true };
  ...
  for (const coord of coords) {
    ...
    const geometry = new THREE.BoxGeometry(
      BUILDING_COLLIDER_SIZE,
      BUILDING_COLLIDER_HEIGHT,
      BUILDING_COLLIDER_SIZE
    );
    ...
    collider.name = buildingId + '_click_collider_' + coord;
    collider.position.set(
      tilePosition.x,
      tilePosition.y + BUILDING_COLLIDER_Y_OFFSET,
      tilePosition.z
    );
    collider.userData = { clickId: buildingId, clickable: true };
    group.add(collider);
  }
  ...
  const model = loadedModels[buildingId];
  if (model) {
    group.visible = model.visible !== false;
    setVisualModelClickable(model, buildingId, false);
    updateRaycastRoot(buildingId, false);
  }

  scene.add(group);
  buildingColliderRoots.set(buildingId, group);
  raycastRoots.add(group);
  requestRender();
}
...
function removeModel(id) {
  ...
  delete positionAnimations[id];
  removeBuildingClickColliders(id);
  const model = loadedModels[id];
  if (model) {
    scene.remove(model);
    ...
    const sourceUrl = modelUrlById.get(id) || model.userData?.sourceUrl;
    modelUrlById.delete(id);
    disposeUnusedGltf(sourceUrl);
    if (id === 'main') {
      tilePositionCache.clear();
    }
    pruneGltfCacheIfNeeded();
...
function setModelVisible(id, visible) {
  const model = loadedModels[id];
  const colliderGroup = buildingColliderRoots.get(id);
  if (model) {
    if (model.visible === visible) {
      if (colliderGroup && colliderGroup.visible !== visible) {
        colliderGroup.visible = visible;
        requestRender();
      }
      return;
    }
...
function setObjectClickState(id, clickable) {
  ...
  const colliderGroup = buildingColliderRoots.get(id);
  if (colliderGroup) {
    setVisualModelClickable(model, id, false);
    updateRaycastRoot(id, false);
    colliderGroup.visible = clickable && model.visible !== false;
    if (clickable) {
      raycastRoots.add(colliderGroup);
    } else {
      raycastRoots.delete(colliderGroup);
    }
    requestRender();
    return;
  }
  ...
  setVisualModelClickable(model, id, clickable);
  updateRaycastRoot(id, clickable);
}
...
function handleCommand(event) {
  ...
  switch (cmd) {
    ...
    case 'syncBuildingClickColliders': {
      const [, buildingId, coordsCsv] = parts;
      syncBuildingClickColliders(buildingId, coordsCsv || '');
      break;
    }
...
String _resolveWebAssetUrl(String url) {
